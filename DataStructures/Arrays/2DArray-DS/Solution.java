 ...
 * 
 * Space complexity: O(1) ~ O(16)  we need 16 contiguous spaces (the 2D array) + 3 int to store the sums
 * Time complexity:  O(1) ~ O((6/4) * [(6/4)(2*2)]) = O((6/4) * (6))   
 * 
 */
public class Solution {
    ...
    static int hourglassSum(int[][] arr) {
        ...
        for (int i = 0; i < 4; i++) { // O(16/4)
            for (int j = 0; j < 4; j++) { // O(16/4)
